...
- **Simulation benchmark vignette:** `vignette("benchmark-simulation-recovery", package = "tidyILD")` — documents the **`ild_simulate()`** DGP; fixed-effect **recovery** (bias, RMSE, nominal Wald coverage) using **`ild_power(..., return_sims = TRUE)`**; connection to empirical **power**. Illustrative scope: Gaussian **`ild_lme()`** path, not exhaustive validation of all backends.
