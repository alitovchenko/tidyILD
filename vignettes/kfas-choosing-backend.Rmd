---
title: "Choosing between lme/nlme, brms, and KFAS"
author: "Alex Litovchenko"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Choosing between lme/nlme, brms, and KFAS}
...
- **`ild_kfas()`** when the **scientific target** is **latent temporal state** estimation (level/trend in state space) for a **single** series per fit, with diagnostics built around **innovations** and the diagnostics bundle.
...
| | **`ild_lme()`** (lme4 / nlme) | **`ild_brms()`** | **`ild_kfas()`** (KFAS) |
|---|----------------|------------------|-------------------------|
| **Typical use** | Multilevel regression, AR1/CAR1 on residuals | Hierarchical Bayes, flexible priors | Univariate latent level (v1), single `.ild_id` per fit |
| **ID structure** | Multiple `.ild_id`, random effects | Multiple `.ild_id`, partial pooling | One person per fit (v1); not pooled latent state across IDs |
| **Temporal structure** | Residual AR1/CAR1; spacing-informed choice | Flexible time / residual modeling | Discrete-time local level on observation order |
| **Priors** | N/A (frequentist) | Yes (user-specified) | Implicit via likelihood / ML |
| **Predictive checks** | Via bundle / extensions | PPC via **brms** (`pp_check`) | Forecast / innovation plots (not PPC); see `ild_autoplot(bundle)` for KFAS |
| **Provenance / report** | `ild_provenance`, `ild_report` | Same | Same; KFAS stores state spec, smoothing, `fit_context` |
...
`inst/dev/KFAS_V1_BACKEND.md` (backend scope), `?ild_kfas`, `?ild_lme`, `?ild_brms`.
